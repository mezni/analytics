...
pub fn get_pool(db_name: &str) -> Result<DbPool, AppError> {
    ...
    Pool::builder().max_size(5).build(manager).map_err(|e| AppError::R2D2Error(e))
}

pub fn initialize_database(pool: &DbPool) -> Result<(), AppError> {
    let conn: PooledConnection<SqliteConnectionManager> = pool.get().map_err(|e| AppError::R2D2Error(e))?;

    let sql_script = fs::read_to_string("database.sql").map_err(|e| AppError::IoError(e))?;

    conn.execute_batch(&sql_script).map_err(|e| AppError::RusqliteError(e))?;
